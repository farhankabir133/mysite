...
const ChatWindow: React.FC<ChatWindowProps> = ({ isOpen, onClose }) => {
  ...
  const handleSendMessage = async (text?: string) => {
    const messageContent = (typeof text !== 'undefined') ? text : inputValue.trim();
    ...
    analytics.track('message_sent', { message_length: userMessage.content.length });

    try {
      const connectKeywords = ['hire', 'contact', 'connect', 'work together', 'collaborate', 'project'];
      if (connectKeywords.some(k => userMessage.content.toLowerCase().includes(k)) && !leadCaptured) setShowLeadForm(true);

      const result = await getGeminiResponse([...messages, userMessage]).catch(() => ({
        response: "Sorry — I couldn't fetch a response right now. Please try again later or contact Farhan directly.",
      ...
      if (isSpeakingEnabled && !speaking) { try { speak({ text: result.response }); } catch { } }
  ...
  const minimizedClasses = 'fixed bottom-4 right-4 w-80 h-16 rounded-t-2xl z-50';
  const maximizedClasses =
  'fixed bottom-4 right-4 sm:bottom-24 sm:right-6 sm:w-[400px] sm:h-[75vh] w-[90vw] h-[65vh] z-50 rounded-t-2xl';
        ...
        <div className="w-full h-full rounded-2xl shadow-2xl backdrop-blur-xl border flex flex-col bg-white dark:bg-slate-800 transition-colors duration-300 overflow-hidden">
          ...
          <div className="flex items-center justify-between p-3 border-b border-gray-200/50 dark:border-slate-700/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-base text-gray-900 dark:text-white">Kabir's AI Assistant</h3>
                <p className="text-xs text-gray-500 dark:text-slate-400">Online • Powered by Gemini</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`p-2 rounded-lg transition-colors ${isSpeakingEnabled ? 'bg-blue-500 text-white' : 'text-gray-600 hover:bg-gray-100 dark:hover:bg-slate-700'}`}
              >
              ...
              <button onClick={onClose} title="Close chat" className="p-2 rounded-lg text-gray-600 hover:bg-gray-100 dark:hover:bg-slate-700">
                <X className="w-4 h-4" />
              </button>
            </div>
          </div>
            ...
            <>
              ...
              <div className="p-2.5 sm:p-3 md:p-4 border-t border-gray-200/50 dark:border-slate-700 flex items-center gap-2">
                ...
                <button
                  ...
                  className={`p-3 rounded-xl border ${isListening ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-700'} hover:bg-blue-500 hover:text-white transition-colors dark:bg-slate-700 dark:text-white`}
                  title="Voice input"
                >
